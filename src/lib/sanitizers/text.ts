...
class HandleText {
     ...
     */
    snakeCaseConvert = (str: string, underline: boolean = true): string => {
        if (!str) return str;
    
        let result = str;
    
        // If underline is true, replace underscores with spaces
        if (underline) {
            result = str.replace(/_/g, ' ');
        }
    
        // Convert to space-separated words and capitalize the first letter
        return result
