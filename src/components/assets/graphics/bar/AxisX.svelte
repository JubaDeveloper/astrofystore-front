...
<g class="axis x-axis" class:snapLabels>
  {#each tickVals as tick, i (tick)}
    ...
    <g class="tick tick-{i}" transform="translate({$xScale(tick)},{Math.max(...$yRange)})">
      {#if gridlines === true}
        <line  class="gridline stroke-sub-300" x1={halfBand} x2={halfBand} y1={-$height} y2="0" />
      {/if}
      {#if tickMarks === true}
        <line
          class="tick-mark fill-sub-600"
          x1={halfBand}
          x2={halfBand}
          y1={tickGutter}
        ...
        />
      {/if}
      <text class="text-[12px] fill-sub-600" x={halfBand} y={tickGutter + tickLen} {dx} {dy} text-anchor={textAnchor(i, snapLabels)}
        >{format(tick)}</text
      >
    </g>
  {/each}
