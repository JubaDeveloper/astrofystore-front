<script lang="ts">
  ...
  import { DenunciaMotivosDto, DenunciaStatusDto, type DenunciaDto } from "$lib/api/modules/denuncias/denuncias.dto";
  ...
  import sinalizador from "$lib/utils/signal.svelte";
    ...
    async function listar() {
      loading=true
      const [r, e] = await denunciaAdminController.listar({
        ...filtros,
        pagina,
...
<div class="flex flex-col justify-between grow h-full">
      ...
      {#each lista as denuncia, i}
        ...
        >
          <div class="flex gap-2 items-center {headers[0].colSpan}">
            <Avatar src={denuncia.loja.logo} name={denuncia.loja.nome}/>
            {denuncia.loja.nome}
          </div>
          <div
          ...
          <div class="flex {headers[3].colSpan}">
            {handleDate.formatISODate(denuncia.criadoEm as string)}
          </div>
          <div class="flex justify-between gap-2 {headers[4].colSpan}">
        ...
        <ModalDenuncia bind:open={openModal[i]} idDenuncia={denuncia.id}/>
      {/each}
    </div>
  {/snippet}
  ...
  {#snippet filtrosSec()}
    <div class="flex w-full justify-between items-center gap-4">
      <BotaoDeslizante bind:value={filtros.motivo} opcoes={DenunciaMotivosDto}/>
      <div class="flex items-center gap-3">
        ...
        <SelectBlock placeholder="Status" options={DenunciaStatusDto} onSelect={()=>{
            pagina=1
        }} bind:value={filtros.status} focusCss="relative grow min-w-[160px]"/>
