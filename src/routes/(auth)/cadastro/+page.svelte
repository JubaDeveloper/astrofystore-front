<script lang="ts">
    ...
    import { formCadastro, type FormCadastro } from "$lib/api/modules/cadastro/cadastro.form.svelte";
    import storageService from "$lib/utils/storageService";
    
    let form = $state<FormCadastro>(formCadastro)
    ...
    const cadastrar = async()=>{
        ...
        const [r,e] = await cadastroController.cadastrar(form)
        ...
        toast.push("Notificação do sistema", "Valide seu e-mail para concluir seu cadastro!")
        loading = false
        goto("/autenticacao")
    }
</script>
    ...
    <div class="w-[440px] flex flex-col items-center">
        ...
        <div class="grid gap-x-4 w-full h-[40px]">
            <button class="text-14 font-normal text-strong-950 flex items-center gap-2" onclick={()=> form.aceitouTermos = !form.aceitouTermos}>
                <Check checked={form.aceitouTermos}/>
